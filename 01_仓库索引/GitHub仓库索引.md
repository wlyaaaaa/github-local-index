...
| wlyaaaaa/Key | PRIVATE | main | V:\Personal\Projects\Key | `main` ahead/behind `0/2`，脏工作区 1 项（cached） | 先同步远端变更，再决定是否提交或推送 |
...
| wlyaaaaa/PCConfig | PRIVATE | main | E:\PCConfig | `main` 已同步，`0/0`（cached）<br>`codex/personalos-shadow-root-fact` 由 PersonalOS 外部 owner 治理，不纳入 Codex 收敛判断（cached）<br>`origin/secret-broker-backup` 仅有 remote-tracking branch ref，默认分支缺少 4 个提交（cached） | 无；由 PersonalOS owner 管理<br>验证后将独有提交整合到仓库实际默认分支<br>正常维护 |
