...
| wlyaaaaa/Key | `main` 已同步，`0/0`，脏工作区 1 项（cached） | 脏工作区 1 项；cached 远端引用 |
...
| wlyaaaaa/PCConfig | `main` 已同步，`0/0`，脏工作区 39 项（cached）<br>`codex/personalos-shadow-root-fact` 由 PersonalOS 外部 owner 治理，不纳入 Codex 收敛判断（cached） | 脏工作区 39 项；cached 远端引用 |
