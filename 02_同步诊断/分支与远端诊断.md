...
| 仓库 | 分支状态 | 原因 |
| --- | --- | --- |
| wlyaaaaa/.agents | `main` 已同步，`0/0`，脏工作区 2 项（live）<br>`codex/local-gpu-arbitration` 已同步，`0/0`（live） | 脏工作区 2 项 |
| wlyaaaaa/github-local-index | `main` 已同步，`0/0`（cached） | cached 远端引用；fetch_failed |
| wlyaaaaa/OpenClawGateway | `codex/local-gpu-arbitration` 已同步，`0/0`（cached）<br>`main` 已同步，`0/0`（cached） | cached 远端引用；fetch_failed |
| wlyaaaaa/PCConfig | `codex/local-gpu-arbitration` 已同步，`0/0`（live）<br>`main` 已同步，`0/0`，脏工作区 10 项（live） | 脏工作区 10 项 |
| wlyaaaaa/wechat-history-ai-bridge | `master` 已同步，`0/0`（live）<br>`codex/weflowbridge-ai-integration-1-0` 已同步，`0/0`（live）<br>`codex/weflowbridge-ai-readme-v2` 无 upstream（live）<br>`codex/weflowbridge-star-readme` 已同步，`0/0`（live）<br>`codex/wechat-history-ai-bridge-branding` 已同步，`0/0`（live） | 无 upstream |
