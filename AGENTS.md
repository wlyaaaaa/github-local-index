...
- 系统项目和受保护仓库由已登记最高权限智能体按真实目标语义审查；只有它能决定是否需要人类因子，adapter 不得按 effect 类型自行派生。人类因子固定为 Passkey、TOTP、Recovery、Account 四类，Google/Microsoft 只是 Account provider；稳定仓库身份、动作分类、正常变化/恶意篡改和受保护 adapter 见 [`git.protected-major-actions`](./docs/contracts/git.protected-major-actions.md)。
