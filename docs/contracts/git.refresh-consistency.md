...
`-MigrateBaseline` 仅保留 explicit bootstrap/repair 兼容；已有有效 v3 baseline 时，默认 full refresh 在同一用户命令内先原子发布 generation 并刷新 private clone navigation，再用 fresh owner inventory 原子 advance baseline/readback。previous→current transition 保留为 nonblocking history，不要求第二次相同 migration。Fast compatibility mode 写 private log；CheckOnly 用 system temp。默认 full refresh 重建 Markdown，fetch 最多尝试三次；commit-pinned snapshot 只刷 metadata。仅 `-ZeroFetchAtomic` 传 `SkipFetch`，复用 atomic generation/manifest/projection readback/pointer CAS/rollback；禁与 Fast/CheckOnly 合用，非 refs freshness/publication 证据。byte-hashed pointer、generation 和 projections 由 `.gitattributes text eol=lf` 固定为 UTF-8/LF，避免 checkout 行尾改写破坏闭包。
