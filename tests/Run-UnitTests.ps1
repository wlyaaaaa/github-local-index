...
if ($hookParameters -contains 'RepoPath') {
    ...
    try {
        ...
        $missingHooks = Join-Path $hookRepo 'uninstalled-hooks'
        & git -C $hookRepo config core.hooksPath 'uninstalled-hooks'
        $inspection = & $hookPath -RepoPath $hookRepo -Inspect
        Assert-Equal 'missing' $inspection.status 'inspection reports a missing active hook'
        Assert-Equal $null $inspection.actual_sha256 'missing hook has no actual hash'
        Assert-Equal (Join-Path $missingHooks 'pre-commit') $inspection.hook_path 'inspection respects the configured hooks directory'
        Assert-True (-not (Test-Path -LiteralPath $missingHooks)) 'inspection does not create the hooks directory'
        & git -C $hookRepo config --unset core.hooksPath
        ...
        foreach ($expectedStatus in @('current', 'drift')) {
            if ($expectedStatus -eq 'drift') { Add-Content -LiteralPath $installedHook -Value '# drift fixture' -Encoding utf8 }
            $beforeHash = (Get-FileHash -LiteralPath $installedHook -Algorithm SHA256).Hash
            $beforeWrite = (Get-Item -LiteralPath $installedHook).LastWriteTimeUtc
            $inspection = & $hookPath -RepoPath $hookRepo -Inspect
            Assert-Equal $expectedStatus $inspection.status "inspection reports $expectedStatus"
            Assert-Equal $firstHookHash $inspection.expected_sha256 'inspection compares against the canonical generated bytes'
            Assert-Equal $beforeHash $inspection.actual_sha256 'inspection reports the actual bytes'
            Assert-Equal $beforeHash (Get-FileHash -LiteralPath $installedHook -Algorithm SHA256).Hash 'inspection does not rewrite the hook'
            Assert-Equal $beforeWrite (Get-Item -LiteralPath $installedHook).LastWriteTimeUtc 'inspection preserves the write time'
        }
        & pwsh -NoProfile -ExecutionPolicy Bypass -File $hookPath -RepoPath $hookRepo | Out-Null
