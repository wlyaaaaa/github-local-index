...
try {
    ...
    if ($tmpSize -ge $originalSize) {
        ...
        throw "Validation failed: Temp file size ($tmpSize) is smaller than original ($originalSize)."
    }
}
catch {
    ...
    Write-Error "❌ Failed to prepend log safely: $_"
    exit 1
}
...
function Test-SecretsLeak {
    ...
    foreach ($file in $gitFiles) {
        # 排除白名单
        if ($file -in $whitelistFiles -or $file -match "tools/Add-PushRecord.ps1") { continue }
...
try {
    ...
    git add "03_推送决策/已推送记录.md"
    
    $commitMsg = "docs: log push of $Repo ($Commit)"
    git commit -m $commitMsg
    
    if ($PushIndex) {
        Write-Host "🚀 Pushing index repository to remote..." -ForegroundColor Cyan
        git push
    } else {
